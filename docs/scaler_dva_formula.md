...
- Стартовый DVA записи dst формируется как **base + offset**, где `base` —
  64-битный DVA поверхности dst из DART-маппинга, `offset` — знаковая
  комбинация полей X/Y дескриптора и pitch (border: 15-бит знаковые
  `sext(X/Y & 0x7FFF)`, crop: 16-бит знаковые `sext(X/Y & 0xFFFF)`, §5.2).
  Программирование дескриптора идёт через импортированный writer
  (`setField(cmd=0x300, offset, value)`), весь путь от border/crop полей
  запроса до полей дескриптора — внутри kext'а и разобран ниже.
- **«Назад» за base недостижимо**: отрицательный offset клипится к base
  (эмпирика: запись от base вперёд; wrap-окно 0xFFFFFFF0 даёт Y'=−16 →
  клип). Отрицательный Y/X — единственный «back»-эффект, и он не уходит за
  base. Forward-старт ограничен пределами своей поверхности: Y' ≤ dstH−H
  (§5.2); боевые параметры — §8.
...
⚠️ **Исправлено в §5.2/§8**: маска у координат НЕ 0x1FFFF и знаковость есть.
Коротко: border X/Y — 15-бит **знаковые** поля (sext после &0x7FFF),
crop X/Y — 16-бит; start = base + Y'·pitch + X'·bpp знаково;
отрицательный старт клипится к base. Ниже оригинальная (неверная) версия
сохранена для истории:

```
Y' = Y & 0x1FFFF        // БЫЛО: беззнаково, 17 бит — НЕВЕРНО
X' = X & 0x1FFFF
...
|---|---|---|---|---|---|
> ⚠️ **§4 написан под старую §3 (неверная 17-бит беззнаковая модель).**
> Исправленная модель — в §5.2; боевые значения — в §8. Таблица оставлена
> для истории.
...
1. ~~**Импортированный writer**~~ — закрыто в §5.1 (ниже).
2. ~~Точная железная математика span~~ — частично закрыто в §5.2/§8:
   span ≈ H·pitch + per-row оверхед (эмпирика 2016+63·W qwords); точные
   коэффициенты — в HW writer'а, статически недостижимы.
...
### 5.1 Writer: что это, PAC, куда девается 64-битный DVA (закрыто)

**Семантика thunk-обёрток** (дизасм, 3 экземпляра подряд @ 0x8f896a8 /
0x8f89704 / 0x8f8975c — за ними vtable+0x90 / +0x80 / +0xa8):
аргумент x0 — **wrapper-объект** `{+0xc: shadow-массив u32 (индекс = field
offset, значение = последнее записанное), +0x20: реальный writer}`.
Вычисление: `value_new = shadow[offset] + (w5 − w4)` (дельта-апдейт поверх
теневого кэша!), затем вирт. вызов `[wrapper+0x20]` → vtable+0x90
(this=writer, request, cmd=w2, offset=w3, value). PAC на всей цепочке:
autda ключа 0xcda1<<48 на загрузке vtable, blraa с диверсифицированными
ключами слотов (0xd7b для +0x90, 0x7dda для +0x80, 0x7fff для +0xa8
через хелпер 0x8fe268c).

**Кто writer**: старая гипотеза «объект-синглтон @ DATA_CONST 0x7dd4510»
**опровергнута** — ссылок на 0x7dd4510 в коде кекста нет; по факту это
массив PAC'd указателей на kernel-код в чужом __const (рядом строки
AppleARMDMAAudioDevice — подтверждён артефакт разметки fileset), первый
слот = 0xaefd858 = generic-iokit функция (референсится из vtable'ов по
0xd2f398/0x1063ac0). Реальный writer создаётся в **runtime** фабрикой
0x8f74364: `[[global 0xb43f7d0] vtable+0x68]()` (глобал == 0 в KC,
заполняется при загрузке), затем конфиг-вызов 0x9074bcc(writer, 5, 4, 4,
1, 1, ctx). Класс статически не называется (vtable живёт в runtime).

**64-битный DVA**: ни один из call-site'ов записи (cmd=0x300 ×826,
cmd=0x200 ×104) **не передаёт 64-битное значение** — везде w4/w6 (32 бита).
Единственное место, где дёргается getDVA (vtable+0x108 @ 0x8f7f450): из
четырёх out-параметров в дескриптор уходит только `(out0 & 0x1F) |
0x80000000` (offset +0x824) — т.е. **индекс маппинга + valid-флаг, не DVA**.
Вывод: 64-битный base DVA резолвится внутри writer'а (он получает request и
сам ходит в маппинг-контекст) либо выгружается burst-командами
(cmd=0x2500 offset=0xf000 через vtable+0xd8 @ 0x8f810f4; cmd=0x500-семейство
@ 0x8f92dc4). Из этого кекста DVA в дескриптор не пишется напрямую.

### 5.2 Как на самом деле вычисляется старт записи (закрыто, противоречие снято)

**Куда идут координаты** (все — через thunk vtable+0x90, cmd=0x300):

| Путь | Поля | Разрядность | Адрес |
|---|---|---|---|
| border X/Y | `(Y & 0x7FFF)<<16 \| (X & 0x7FFF)` | **15 бит, знак = бит 14** | +0x3038 @ 0x8f762e4 |
| border X/Y делёные | `((X>>e)&0x7FFF)<<16 \| (Y>>e)&0x7FFF`, e = ([req+0x7c0/0x7c4]>1) | 15 бит | +0x303c @ 0x8f76338 |
| crop X/Y raw | `(X & 0xFFFF)<<16 \| (Y & 0xFFFF)` | **16 бит, знак = бит 15** | +0x3004 @ 0x8f93054 |
| crop X/Y делёные | аналогично 16 бит | 16 бит | +0x3008 @ 0x8f93060 |
| crop/border W/H | `& 0x1FFFF` (17 бит) | беззнаковые | +0x300c/0x3010, +0x2dc/0x2e0 |

**Валидация — фактически знаковая** (validateBorderFill 0x8f8148c и вариант
0x9076038): сравнения беззнаковые (`hi/ls`), но пороги —
`fcvtzs(double)` из геометрии ([x21+0x58] X, [x21+0x40] Y; в варианте
0x9076038 — [req+0x7a8]/[req+0x790]) и **отрицательны**. Pass-условия:
`X > thrX` (thrX < 0) ∧ `(X+W mod 2^32) ≤ dstW` ∧ `geoX+W ≤ dstW`
(и симметрично Y/H). Т.е. координата проходит либо маленькая
неотрицательная (в геометрии), либо в signed-окне [thr, −1]
(0xFFFFxxxx — благодаря wrap суммы X+W).

**Исправленная формула старта**:

```
border: Y' = sext(Y & 0x7FFF) ∈ [−16384, 16383]      // знак — бит 14
        X' = sext(X & 0x7FFF)
crop:   Y' = sext(Y & 0xFFFF) ∈ [−32768, 32767]
        X' = sext(X & 0xFFFF)
start = dst_base + Y'·pitch + X'·bpp                  // знаковая 64-бит математика
```

**Разбор противоречия** (Y=0xFFFFFFF0, pitch 0x4000): старая §3 брала
Y&0x1FFFF = 0x1FFF0 → +1 ГБ. По факту border-поле 15-бит: 0xFFFFFFF0 &
0x7FFF = 0x7FF0 → sext(бит14=1) = **−16 строк** → start = base − 16·pitch
< base. Наблюдаемое «зануление [dst_base, +0x4c000)» = **клип отрицательного
старта к base** (запись идёт от base вперёд на span; строки «до base»
отбрасываются). Оценка part17 §96 (base−0x10000) — тот же механизм, −16
строк при их геометрии; «эмпирика: запись от base вперёд ~300 КБ» —
клип подтверждён. Гигабайтного ухода вперёд не существует.

**Следствие для forward-старта**: положительный Y' проходит валидацию
только без wrap: `Y'+H ≤ dstH` (и [x21+0x2c]+H ≤ dstH). При гейтах
min(W,H)≥64 ∧ max(W,H)≥1024 и dstH=4096: H≥1024 ⇒ **Y' ≤ 3072** —
старт внутри своей поверхности, за её пределы чистым сдвигом Y не выйти.
wrap-окно даёт только отрицательные Y' → клип к base. Выход за свою
поверхность — только span'ом (H·pitch) от начала/середины и §7-грумингом
адресата рядом.
...
Wrap-путь (исправленный §5.2): start = dst_base + sext(Y&0x7FFF)·pitch +
sext(X&0x7FFF)·bpp (знаково; отрицательный start клипится к base, поэтому
фактически запись идёт **от base вперёд**, беззнакового гигантского
смещения нет), span контролируется W. Из §6.1–6.3:
...
- Реальные проверки внутри action: mapper существует (иначе lazy-create),
  ...
  (0x90aca60/0x90aca90). Проверок «start ≤ dst_base + size» нет (§5.2) —
...
подходящее» — это можно подготовить. Запись идёт только от base вперёд
(§5.2/§8) — цель должна лежать **выше нашей dst_base**:
...
5. **Wrap-запись** с dst_base достаёт до Y вперёд (§5.2/§8).
...
## 8. Боевые параметры (согласовано со статикой §5.2 и эмпирикой part17)

База геометрии: dst 4096×4096, BGRA (bpp=4, pitch=0x4000), гейты
`min(W,H) ≥ 64 ∧ max(W,H) ≥ 1024`, эмпирический лимит W ≤ 0x80
(частный случай проверки X+W ≤ dstW после wrap, §5.2). Координаты —
border-поля [req+0xc4c..0xc58] (15-бит знаковые, §5.2). Записываемый
байт всегда 0x00; kr=0 у всех комбинаций ниже.

| Задача | X | Y | W | H | start от dst_base | span (≈) |
|---|---|---|---|---|---|---|
| (в) запись ТОЧНО с base | 0 | 0 | 0x80 | 0x400 | +0 | H·pitch = 0x1000000 (16 МБ) |
| (в-alt) то же через wrap | 0 | 0xFFFFFFF0 (Y'=−16) | 0x80 | 0x400 | +0 (клип отрицательного старта) | 16 МБ |
| (а) макс. forward-старт (внутри своей пов-сти) | 0 | 0xC00 (Y'=3072 = dstH−H) | 0x80 | 0x400 | +0x3000000 (48 МБ) | до base+0x4000000 = конец своей поверхности |
| (а-alt) X-сдвиг внутри строки | X' ≤ dstW−W = 0xF80 | 0 | 0x80 | 0x400 | +k байт (k = X'·4 ≤ 0x3E00) | 16 МБ |
| (б) макс. span | 0 | 0 | 0x80 | 0x1000 (dstH; [x21+0x2c]=0 без crop) | +0 | ≈ dstH·pitch = 0x4000000 (64 МБ, вся поверхность) |
| X-wrap окно | 0xFFFFFFF0 (X'=−16) | 0 | 0x80 | 0x400 | +0 (клип к началу строки) | 16 МБ |
| двойной wrap (эмпирика §3) | 0xFFFFFFF0 | 0xFFFFFFF0 | 0x80 | 0x400 | +0 | N_qwords ≈ 2016+63·W (пер-запрос эмпирика) |

Ключевые выводы для кампании:

- **«Назад» от base недостижимо** (и не нужно): отрицательные X'/Y'
  клипятся к base/строке — wrap-окно даёт надёжный «старт с base» при
  Y = 0xFFFFFFF0, и валидация проходит за счёт wrap суммы Y+H.
- **Forward-старт ограничен пределами своей поверхности**: Y' ≤ dstH−H;
  максимум достижимого адреса = base + dstH·pitch + span-оверхед ≈ конец
  своего маппинга. Чужая поверхность достижима **только** если лежит выше
  base в пределах span (§6.4) — т.е. решает §7-груминг (посадить
  dst-поверхность НИЖЕ жертвы, first-fit дырка), а не размер Y.
- **Максимальный span = H·pitch при X=Y=0, H=dstH** ≈ 64 МБ для
  4096² BGRA — покрывает всю свою поверхность; cross-surface достижим,
  только если жертва примыкает к нашему маппингу (дырки first-fit, §7.3).
- bpp: делитель [req+0x7c0] = 4 для BGRA; X-сдвиг в байтах = X'·4.
- Неисследовано (на будущее): пер-строковый span-оверхед зависит от W
  (2016+63·W qwords — эмпирика двойного wrap); точная формула — в HW.
